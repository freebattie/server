...
export function locations() {
  ...
  route.put("/", async (req, res) => {
    ...
    try {
      ...
      const message = {
        ...
        payload: { city: loc.city, location: loc.location },
      };
  ...
  route.get("/:location/data/:time", async (req, res) => {
    ...
    console.log("testing", timeSpan.toUTCString());
    let dataIntervl = await liveDataModel
      .find({
        location,
        createdAt: { $gte: timeSpan.toUTCString() },
      })
      .catch((e) => console.log("error", e));
    console.log("what here ", dataIntervl);
    if (dataIntervl.length < 6) {
      dataIntervl = await liveDataModel
        .find({ location })
        .sort({ _id: -1 })
        .limit(time * 6);
    }
